...
int dynamic_programming_td(vector<array<int, 2>>& tape, int resistance) {
    ...
    int tmp = _dynamic_programming_td(tape, m, resistance, 0);
    return tmp;
}
...
int _dynamic_programming_td(vector<array<int, 2>>& tape, vector<vector<int>>& m , int R, int cur_elem){
    ...
    if(m[cur_elem][R]==-1) m[cur_elem][R] = max(_dynamic_programming_td(tape, m, R, cur_elem+1),
                                                1+_dynamic_programming_td(tape, m, min(R-tape[cur_elem][WGH_INDEX], tape[cur_elem][RES_INDEX]), cur_elem+1));
    return m[cur_elem][R];
}